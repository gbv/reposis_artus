...
const allowedGenreIds = ["book", "article", "collection", "thesis","dissertation","unpublished_dissertation"];
const allowedHostIds = ["journal","collection","series"];
    ...
    if (!(typeof host === 'undefined')) {
        ...
        $.each(hosts, function (ind, val) {
            if (val == 'standalone') {
                ...
                console.log(val);
                if (allowedHostIds.indexOf(val) === -1) return;
...
function createGenreOption(category, level) {
    ...
    // skip disallowed IDs
    if (allowedGenreIds.indexOf(id) === -1) return "";
