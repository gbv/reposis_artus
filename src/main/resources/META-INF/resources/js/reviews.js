$(document).ready(function () {
    ...
    if (genreSelect) {
        ...
        if (reviewOption) reviewOption.remove();
            ...
            //if it was selected before reset publikationstyp to "article"
            const emptyOption = genreSelect.querySelector('option[value="article"]');
    ...
    saveButton.addEventListener("click", function () {
        const titleInput = document.querySelector(
            'input[name="/mycoreobject/metadata[1]/def.modsContainer[1]/modsContainer[1]/mods:mods[1]/mods:titleInfo[1]/mods:title[1]"]'
        );

        const noteTextarea = document.querySelector(
            'textarea[name="/mycoreobject/metadata[1]/def.modsContainer[1]/modsContainer[1]/mods:mods[1]/mods:note[1]"]'
        );

        const reviewedTitleInput = document.querySelector(
            'input[name*="relatedItem"][name*="titleInfo"][name*="mods:title"]'
        );

        const authorInputs = document.querySelectorAll(
            'input[ref_key="searchBox"]'
        );
        ...
        const bookTitle = reviewedTitleInput
            ? reviewedTitleInput.value.trim()
            : "";
        ...
        if (noteTextarea) {
            ...
            noteTextarea.value =
                "The title of this metadata was automatically generated";
